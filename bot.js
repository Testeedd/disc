...
class Bot {
    ...
    processMessage(msg) {
        if(msg.content.startsWith("ш!радуга")) {
            for(var role of msg.mentions.roles.array()) {
                msg.reply(`добавленна роль " + role + " в лист радужных ролей. 
___|LLIAJIYH-BOT| PROJECTS Все права защищены.___`);
    ...
    addRainbowRole(guild, role) {
        ...
        for(var existingRole of this.servers[guild]) {
            if(existingRole == role) {
                return "Эта роль уже добавленна!";
            }
        }
